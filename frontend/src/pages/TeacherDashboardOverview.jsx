  ...
  useEffect(() => {
    ...
    const fetchDashboardData = async () => {
      try {
        ...
        console.log('Teacher dashboard data:', mockData);
        setStats(mockData);
      } catch (e) {
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
            {statCards.map(card => {
              ...
              return (
                <div key={card.key} className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 flex flex-col items-center hover:shadow-xl transition-shadow border border-gray-100 dark:border-gray-700">
                  ...
                  <div className="text-3xl font-bold text-gray-900 dark:text-white">
                    <AnimatedCounter 
                      ...
                      duration={1500 + (statCards.indexOf(card) * 200)} 
                    />
                  </div>
                  <div className="text-gray-600 dark:text-gray-300 mt-1 text-sm font-medium">{card.label}</div>
                </div>
              );
            })}
          </div>
